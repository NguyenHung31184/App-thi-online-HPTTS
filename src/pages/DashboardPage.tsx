...
function StudentDashboard() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {studentSession?.student_name && studentSession?.student_dob && (
        <p className="text-sm text-slate-600 mb-4 rounded-xl bg-white/80 border border-slate-100 px-4 py-2 inline-block">
          Thí sinh: <span className="font-semibold">{studentSession.student_name}</span> · Ngày sinh:{' '}
          <span className="font-mono">{studentSession.student_dob}</span>
        </p>
      )}
      ...
      {!loading && !error && windows.length === 0 && (
        <p className="text-slate-500">
          Không có kỳ thi nào trong thời gian làm bài.
        </p>
      )}
