-- Phase A reconciliation. Safe to run after the partial P1 schema was created manually.
...
ALTER TABLE public.question_import_jobs
  ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  ADD COLUMN IF NOT EXISTS completed_at TIMESTAMPTZ;

ALTER TABLE public.question_import_drafts
  ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT now();
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'question_bank_status_check') THEN
    ALTER TABLE public.question_bank ADD CONSTRAINT question_bank_status_check CHECK (status IN ('draft', 'review', 'published', 'retired'));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'question_bank_cognitive_level_check') THEN
    ALTER TABLE public.question_bank ADD CONSTRAINT question_bank_cognitive_level_check CHECK (cognitive_level IS NULL OR cognitive_level IN ('recognition', 'comprehension', 'application'));
  END IF;
END $$;

INSERT INTO public.question_libraries (occupation_id, module_id, name, description, created_by)
SELECT q.occupation_id::uuid, q.module_id,
  COALESCE(o.name, 'Ngan hang cau hoi') || CASE WHEN q.module_id IS NULL THEN ' - Chua gan mo-dun' ELSE ' - Mo-dun ' || q.module_id END,
  'Ngan hang tu du lieu cau hoi hien co.', NULL
FROM public.question_bank q
LEFT JOIN public.occupations o ON o.id::text = q.occupation_id::text
WHERE q.library_id IS NULL
  AND q.occupation_id::text ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
...
CREATE INDEX IF NOT EXISTS question_bank_library_status_idx ON public.question_bank (library_id, status, is_deleted);
CREATE INDEX IF NOT EXISTS question_bank_taxonomy_idx ON public.question_bank (taxonomy_node_id, cognitive_level, difficulty);
CREATE INDEX IF NOT EXISTS question_versions_question_created_idx ON public.question_versions (question_id, created_at DESC);
...
CREATE POLICY question_libraries_staff_all ON public.question_libraries FOR ALL TO authenticated
...
CREATE POLICY question_taxonomy_nodes_staff_all ON public.question_taxonomy_nodes FOR ALL TO authenticated
  USING (public.get_my_role() IN ('admin', 'teacher'))
  WITH CHECK (public.get_my_role() IN ('admin', 'teacher'));
DROP POLICY IF EXISTS question_import_jobs_owner_or_staff ON public.question_import_jobs;
DROP POLICY IF EXISTS question_import_jobs_staff_all ON public.question_import_jobs;
CREATE POLICY question_import_jobs_owner_or_staff ON public.question_import_jobs FOR ALL TO authenticated
  USING (requested_by = auth.uid() OR public.get_my_role() IN ('admin', 'teacher'))
  WITH CHECK (requested_by = auth.uid() OR public.get_my_role() IN ('admin', 'teacher'));
DROP POLICY IF EXISTS question_import_drafts_staff_all ON public.question_import_drafts;
CREATE POLICY question_import_drafts_staff_all ON public.question_import_drafts FOR ALL TO authenticated
  USING (public.get_my_role() IN ('admin', 'teacher'))
  WITH CHECK (public.get_my_role() IN ('admin', 'teacher'));
DROP POLICY IF EXISTS question_versions_staff_select ON public.question_versions;
CREATE POLICY question_versions_staff_select ON public.question_versions FOR SELECT TO authenticated
  USING (public.get_my_role() IN ('admin', 'teacher'));
...
CREATE POLICY question_imports_staff_select ON storage.objects FOR SELECT TO authenticated
...
CREATE POLICY question_imports_staff_insert ON storage.objects FOR INSERT TO authenticated
...
CREATE POLICY question_imports_staff_update ON storage.objects FOR UPDATE TO authenticated
  USING (bucket_id = 'question-imports' AND public.get_my_role() IN ('admin', 'teacher'))
  WITH CHECK (bucket_id = 'question-imports' AND public.get_my_role() IN ('admin', 'teacher'));
